...
## Message DAG Architecture Requirements

### Core Structure

Claude Code messages form a Directed Acyclic Graph (DAG):

- `uuid`: Unique message identifier
- `parentUuid`: Optional parent reference
- Multiple children create branches
- Missing parents indicate orphaned messages

### Orphaned Message Handling

Messages with non-existent `parentUuid` values must be treated as root nodes:
...
def identify_active_branch(messages_by_uuid, children_by_parent, parent_uuid):
    """Last child in file represents active branch"""
...
## Compact Operation Requirements

### Architecture

Compact operations maintain session continuity:

- **Trigger**: Manual `/compact` or automatic at ~155k tokens
- **Behavior**: Same file continues with new root
- **Continuity**: Tracked via `logicalParentUuid`
- **Frequency**: Sessions support 8+ compacts
...
class CompactHandler:
    ...
    def process_compact(self, messages):
        """Process messages with compact handling"""
        processed = []
        in_compact = False
        compact_root = None
    ...
    def get_logical_parent(self, uuid):
        """Get logical parent across boundaries"""
        return self.logical_parent_map.get(uuid)
```

## Sidechain Processing Requirements

### Agent Identification

Agent names from Task tool `subagent_type` parameter:

```python
def extract_agent_from_task(assistant_msg):
    """Extract agent from Task invocation"""
...
def match_sidechains_to_agents(messages):
    """Match sidechains to originating agents"""
    sidechain_agents = {}
    pending_task = None
...
Required for OneDrive, Dropbox, and network drives:
...
def read_with_retry(file_path, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.errno == 5 and attempt < max_retries - 1:
                if attempt == 0:
                    logger.warning(f"I/O error reading {file_path} - retrying")
                time.sleep(retry_delay)
                retry_delay *= 2
            else:
                raise
...
class ClaudeCodeSessionProcessor:
    """Complete session processor implementation"""
    ...
    def parse(self):
        """Parse session with edge case handling"""
...
- Retry logic for cloud-synced directories
- Agent names from Task tool parameters
- Compact continuity via `logicalParentUuid`
- File position determines active branches
- Sidechains matched to agents
