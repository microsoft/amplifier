...
## Multi-Model Co-Existence: Tagging and Branching (2026-02-28)

### Issue

Working with two AI platforms (Claude Code and OpenCode/Gemini) in the same repo requires clear distinction of who did what to avoid confusion during review and merge.

### Root Cause

Without clear markers, it's difficult for Claude (acting as architect/reviewer) to distinguish between its own generated code and Gemini's implementation work. Unstructured branch names also lead to cluttered PR lists.

### Solution

**Explicit tagging and namespace separation:**

1.  **Branch Names:** OpenCode uses the `oc/` prefix (e.g., `oc/feature-name`) to clearly identify its branches.
2.  **Code Markers:** Use `// [AI:OPENCODE]` (or appropriate comment syntax) at the top of new files or before significant logic blocks generated by Gemini/OpenCode.
3.  **PR Markers:** Automated provenance markers in PR descriptions (`🤖 Provenance: Created by Gemini via OpenCode.`) and Gitea labels.
4.  **Commit Metadata:** Standard commit footer (`🤖 Generated with OpenCode + Gemini 3.1`).

### Prevention

- Always create implementation branches with `oc/` prefix.
- Tag new logic with `// [AI:OPENCODE]`.
- Use the standard commit message footer and PR marker.
