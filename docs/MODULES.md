...
| **amplifier-drumbeat** | An automation engine for long-running agent sessions — write an automation as a markdown file (a schedule, a notify policy, an ordered list of steps) and drumbeat runs it in a pinned agent session, on time, unattended, on top of amplifier-agent | [amplifier-drumbeat](https://github.com/microsoft/amplifier-drumbeat) |
| **amplifier-smart-tools** | The home of the Smart Tools specification, its machine-checkable conformance kit, and the catalog of reference implementations — the format for packaging domain expertise into tools any agent can use (deterministic paths run with no model; higher-level commands invoke AI under the hood) | [amplifier-smart-tools](https://github.com/microsoft/amplifier-smart-tools) |
| **amplifier-smart-tool-tmux** | A "smart tool" for tmux fleets — a library-first CLI that reports what is happening across every tmux session on a machine (which are parked at a prompt, which finished and how, which need a human) and, under explicit confirmation, types into one or starts one. Deterministic verbs run with nothing configured; model-backed `triage`/`interpret` run through amplifier-agent | [amplifier-smart-tool-tmux](https://github.com/microsoft/amplifier-smart-tool-tmux) |
| **amplifier-smart-tool-team-pulse** | Team Pulse smart tool for the Amplifier project | [amplifier-smart-tool-team-pulse](https://github.com/microsoft/amplifier-smart-tool-team-pulse) |
| **amplifier-drumpack-tmux** | A drumbeat drumpack that puts the tmux-fleet smart tool on an automation's PATH with an agent-facing card — one line in `drumpacks.txt` and a scheduled agent can observe (and, under confirmation, act on) the machine's tmux fleet | [amplifier-drumpack-tmux](https://github.com/microsoft/amplifier-drumpack-tmux) |
...
| **converge** | Vision-first, contract-driven development — direction lands in ratified contracts first, work is derived from the contract-vs-reality gap: reconciliation ledger, candidate-guard hook, owner-gated wave recipes | [amplifier-bundle-converge](https://github.com/microsoft/amplifier-bundle-converge) |
