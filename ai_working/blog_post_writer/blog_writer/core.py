...
class BlogWriter:
    ...
    async def write_blog(
        ...
        Args:
            brain_dump: Original brain dump content
        ...
        if previous_draft and feedback:
            ...
            return await self._revise_blog(previous_draft, feedback, style_profile)
        logger.info("Writing initial blog draft")
        return await self._write_initial(brain_dump, style_profile)

    async def _write_initial(self, brain_dump: str, style_profile: dict[str, Any]) -> str:
        ...
        prompt = f"""Transform this brain dump into a polished blog post:
...
3. Has a compelling title and introduction
4. Flows logically with clear sections
5. Includes a strong conclusion
...
Return ONLY the blog post content in markdown format, starting with # Title."""
    ...
    async def _revise_blog(self, previous_draft: str, feedback: dict[str, Any], style_profile: dict[str, Any]) -> str:
...
3. Keep the overall structure unless specifically asked to change
