...
3. **Dispatch** using Task tool: `Task(subagent_type="amplifier-core:modular-builder", model="sonnet", max_turns=25, ...)`
...
- Spec compliance review → `Task(subagent_type="amplifier-core:test-coverage", model="haiku", ...)`
- Code quality review → `Task(subagent_type="amplifier-core:zen-architect", model="sonnet", ...)` in REVIEW mode
- Security-sensitive tasks → add `Task(subagent_type="amplifier-core:security-guardian", model="opus", ...)` as third reviewer
...
### DAG-Assisted Wave Computation (Preferred)

If the plan contains a `## TaskDAG` section with a JSON block, use it to auto-compute waves instead of manual dependency analysis:

1. **Parse the TaskDAG JSON** from the `## TaskDAG` section of the plan document
2. **Log DAG stats** immediately after parsing:
   ```
   DAG: [total_tasks] tasks, max parallelism [max_parallelism], critical path length [len(critical_path)]
   ```
3. **Auto-compute waves** from `depends_on` edges:
   - Wave 0: Tasks with empty `depends_on` that are `[TRACER]` tasks (run solo)
   - Wave 1: All tasks with `depends_on: []` that are not TRACER (run in parallel)
   - Wave N: Tasks whose `depends_on` tasks are all in waves 0..N-1 (run in parallel)
4. **Confidence check** before dispatching each task:
   - If task `confidence < 0.5`: Dispatch `agentic-search` first to gather context, then re-evaluate
   - If task `confidence >= 0.5`: Proceed with normal dispatch
   - Log: `Task [id] confidence [score] — [dispatching agentic-search first | proceeding]`
5. **Use `parallel` flag**: If a task has `"parallel": false`, run it solo in its wave even if other tasks are in the same wave

**Example DAG parsing output:**
```
DAG: 8 tasks, max parallelism 3, critical path length 4
Wave 0 (solo):     Task 0.1 [TRACER] — confidence 0.7
Wave 1 (parallel): Task 1.1 (conf 0.9), Task 1.2 (conf 0.9), Task 1.3 (conf 0.95)
Wave 2 (parallel): Task 2.1 (conf 0.8), Task 2.2 (conf 0.7)
Wave 3 (sequential): Task 3.1 (conf 0.6 — dispatching agentic-search first)
```

### Manual Wave Analysis (Fallback)

If no `## TaskDAG` section is present, fall back to manual analysis:
