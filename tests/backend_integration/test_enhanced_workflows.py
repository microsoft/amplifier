...
def mock_memory_system():
    ...
    memory_store.search_recent.return_value = [
        Mock(content="Recent memory", category="fact", id="3")
    ]
...
def mock_task_tracker():
    ...
    return {
        ...
        "complete_task": AsyncMock(return_value={
            "task_id": "task_123",
            "status": "completed",
...
def mock_web_research():
    ...
    return {
        ...
        "fetch_url": AsyncMock(return_value={
            "url": "https://example.com",
...
def mock_agent_context_bridge():
    ...
    return {
        ...
        "extract_agent_result": Mock(return_value={
            "agent_output": "Agent completed task",
            "formatted_result": "Formatted result for display"
        })
    }
...
class TestTaskDrivenDevelopmentWorkflow:
    ...
    def test_create_task_and_work_workflow(self, integration_test_project, mock_task_tracker, mock_codex_cli):
        ...
        with patch('amplifier.core.backend.CodexBackend.manage_tasks') as mock_manage_tasks, \
            ...
            mock_manage_tasks.side_effect = [
                ...
                {
                    "success": True,
                    "data": [{"task_id": "task_123", "status": "in_progress"}],
                    "metadata": {"count": 1}
                },
                # complete_task
                {
...
class TestResearchAssistedDevelopmentWorkflow:
    ...
    def test_search_and_fetch_workflow(self, integration_test_project, mock_web_research, mock_codex_cli):
        ...
        with patch('amplifier.core.backend.CodexBackend.search_web') as mock_search, \
...
class TestAgentContextBridgeWorkflow:
    ...
    def test_spawn_agent_with_context_workflow(self, integration_test_project, mock_agent_context_bridge, mock_codex_cli):
        """Test complete workflow: spawn agent with context → verify context passed → integrate results."""
        with patch('amplifier.core.backend.CodexBackend.spawn_agent_with_context') as mock_spawn_with_context, \
             patch('subprocess.run', return_value=mock_codex_cli), \
             patch('os.getcwd', return_value=str(integration_test_project)):
    ...
    def test_context_serialization(self, integration_test_project, mock_agent_context_bridge):
        ...
        with patch('amplifier.core.backend.CodexBackend.spawn_agent_with_context') as mock_spawn, \
             patch('os.getcwd', return_value=str(integration_test_project)):
        ...
        with patch('amplifier.core.backend.CodexBackend.spawn_agent_with_context') as mock_spawn, \
            ...
            mock_spawn.return_value = {
                ...
                "data": {
                    ...
                    "formatted_result": "Clean formatted result for display"
                },
...
class TestAutoQualityCheckWorkflow:
    ...
    def test_auto_check_failing_results(self, integration_test_project, mock_make_check_failure):
        ...
        with patch('subprocess.run', return_value=mock_make_check_failure), \
             patch('os.getcwd', return_value=str(integration_test_project)):
