...
try:
    ...
    from codex.mcp_servers.session_manager.server import initialize_session, finalize_session, health_check as session_health_check
    from codex.mcp_servers.quality_checker.server import check_code_quality, validate_environment
    from codex.mcp_servers.transcript_saver.server import save_current_transcript, list_available_sessions
except ImportError:
    ...
    async def test_check_code_quality_tool_via_mcp(self, integration_test_project, mock_make_check_success):
        ...
        with patch('subprocess.Popen') as mock_popen, \
            ...
            tool_response = {
                ...
                "result": {
                    "success": True,
                    "data": {"status": "passed", "output": "Checks passed"}
                }
            }
            mock_process.communicate.return_value = (json.dumps(tool_response), b'')
            mock_process.returncode = 0
    ...
    async def test_save_current_transcript_tool_via_mcp(self, integration_test_project, mock_codex_session_dir):
        ...
        with patch('subprocess.Popen') as mock_popen:
            ...
            tool_response = {
                ...
                "result": {
                    "success": True,
                    "data": {"export_path": "/path/to/transcript.md"}
                }
            }
            mock_process.communicate.return_value = (json.dumps(tool_response), b'')
            mock_process.returncode = 0
...
class TestMCPServerIntegrationWithCodexCLI:
    ...
    def test_codex_cli_spawns_mcp_servers(self, integration_test_project, mock_codex_cli):
        ...
        with patch('subprocess.run', return_value=mock_codex_cli):
            ...
            result = subprocess.run(
                ["codex", "--config", str(config_path), "--profile", "development"],
                capture_output=True,
                text=True
            )
    ...
    def test_codex_profile_controls_server_selection(self, integration_test_project, mock_codex_cli):
        ...
        for profile, expected_servers in profiles.items():
            with patch('subprocess.run', return_value=mock_codex_cli):
                result = subprocess.run(
                    ["codex", "--config", str(config_path), "--profile", profile],
                    capture_output=True,
                    text=True
                )
