...
class TestClaudeSessionWorkflow:
    ...
    def test_claude_complete_session_workflow(self, integration_test_project, mock_memory_system, mock_claude_cli):
        ...
        with patch('amplifier.memory.MemoryStore', return_value=mock_memory_system["store"]), \
    ...
    def test_claude_session_finalization_timeout(self, integration_test_project, mock_memory_system):
        ...
        with patch('amplifier.extraction.MemoryExtractor', return_value=mock_memory_system["extractor"]), \
...
""")
        ...
        with patch('subprocess.run', return_value=mock_codex_cli), \
            ...
            result = subprocess.run([str(wrapper_script), "--profile", "development"], 
...
""")
        ...
        with patch('amplifier.memory.MemoryStore', return_value=mock_memory_system["store"]), \
            ...
            result = subprocess.run([sys.executable, str(session_init), "--prompt", "Test manual"], 
                                  capture_output=True, text=True)
...
class TestCrossBackendWorkflows:
    ...
    def test_memory_extraction_identical_across_backends(self, integration_test_project, mock_memory_system, sample_messages):
...
class TestSessionWorkflowErrors:
    ...
    def test_session_workflow_with_corrupted_data(self, integration_test_project):
        ...
        with patch('amplifier.memory.MemoryStore') as mock_store_class, \
             patch('os.getcwd', return_value=str(integration_test_project)):
