...
try:
    from .codex.mcp_servers.base import (
        MCPLogger,
        AmplifierMCPServer,
        get_project_root,
    ...
    async def test_initialize_session_with_memories(self, mock_memory_store, mock_memory_searcher):
        ...
        with patch('sys.path', []), \
            ...
            result = await initialize_session(
                prompt="Test prompt for memory search",
...
class TestQualityChecker:
    ...
    async def test_check_code_quality_success(self, temp_project_dir, mock_subprocess):
        ...
        with patch('subprocess.run', return_value=mock_subprocess), \
            ...
            result = await check_code_quality(
                file_paths=["test.py"],
                tool_name=None,
    ...
    async def test_run_specific_checks_lint(self, temp_project_dir):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            result = await run_specific_checks(
                check_type="lint",
                file_paths=["test.py"],
                args=["--fix"]
            )
...
class TestTranscriptSaver:
    ...
    async def test_convert_transcript_format(self):
        ...
        with patch('sys.path', []), \
            ...
            result = await convert_transcript_format(
                session_id="test123",
                from_format="claude",
                to_format="codex"
            )
    ...
    async def test_tool_registration(self):
        ...
        with patch('mcp.server.fastmcp.FastMCP') as mock_mcp:
            ...
            try:
                ...
                from .codex.mcp_servers.session_manager.server import initialize_session, finalize_session, health_check
