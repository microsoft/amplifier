...
def mock_bs4():
    ...
    with patch("codex.mcp_servers.web_research.server.BeautifulSoup") as mock_bs:
        ...
        mock_soup.select.return_value = [Mock(get_text=lambda: "Test Result", get=lambda x: "http://example.com")]
...
class TestWebResearchServer:
    ...
    async def test_search_web_success_ddgs(self, mock_ddgs):
        ...
        with (
            patch("codex.mcp_servers.web_research.server.DDGS_AVAILABLE", True),
    ...
    async def test_search_web_success_requests_fallback(self, mock_requests, mock_bs4):
        ...
        with (
            patch("codex.mcp_servers.web_research.server.DDGS_AVAILABLE", False),
