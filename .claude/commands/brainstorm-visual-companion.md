...
brainstorm-server/start-server.sh --project-dir /path/to/project
...
1. **Start watcher first** (background bash) — avoids race condition:
   ```bash
   brainstorm-server/wait-for-feedback.sh $SCREEN_DIR
   ```
...
```bash
brainstorm-server/stop-server.sh $SCREEN_DIR
```
