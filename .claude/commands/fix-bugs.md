...
- **Gitea operations**: Use Gitea MCP tools (`mcp__gitea__*`) for all Gitea PR operations (create, merge, list, review). Fallback: `tea` CLI (`C:/claude/tools/tea.exe`). See Phase 7b/7d for exact usage.
...
All bugfix scripts default to `ApiBase=http://localhost:5010` and `ApiKey=fusecp-admin-key-2026`. Screenshots save to `tmp/` by default.

**Gitea PR operations** use Gitea MCP tools (preferred) or `tea` CLI (fallback):
- Create PR: `mcp__gitea__create_pull_request(owner="admin", repo="fusecp-enterprise", title="fix: title", body="description", head="hotfix/bug-30", base="main")`
- Merge PR: `mcp__gitea__merge_pull_request(owner="admin", repo="fusecp-enterprise", index={pr-number}, Do="squash")`
- Fallback (tea): `cd /c/claude/fusecp-enterprise && /c/claude/tools/tea.exe pr create --head "hotfix/bug-30" --title "fix: title" --description "body"`
...
Create PR on Gitea using MCP (preferred):

```
mcp__gitea__create_pull_request(
  owner="admin",
  repo="fusecp-enterprise",
  title="fix: {title} (Bug #{id})",
...
The response includes the PR index number. Save it for Phase 7c.

Fallback (tea CLI — use if MCP is unavailable):
```bash
cd /c/claude/fusecp-enterprise && /c/claude/tools/tea.exe pr create \
  --head "hotfix/bug-{id}" \
  --title "fix: {title} (Bug #{id})" \
...
Merge the PR on Gitea using MCP (preferred):

```
mcp__gitea__merge_pull_request(owner="admin", repo="fusecp-enterprise", index={gitea-pr-number}, Do="squash")
```

Fallback (tea CLI): `cd /c/claude/fusecp-enterprise && /c/claude/tools/tea.exe pr merge --style squash {gitea-pr-number}`

Then delete the remote branch:
```bash
cd /c/claude/fusecp-enterprise && git push origin --delete hotfix/bug-{id}
```
