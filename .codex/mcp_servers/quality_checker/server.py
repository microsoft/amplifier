        ...
        async def check_code_quality(
            ...
            try:
                ...
                if not makefile_path.exists() or not make_target_exists(makefile_path, "check"):
                    return error_response("No Makefile with 'check' target found", {
        ...
        async def run_specific_checks(
            ...
            try:
                ...
                if check_type not in command_map:
                    return error_response(f"Unknown check type: {check_type}", {
                        "supported_types": list(command_map.keys())
                    })
        ...
        async def validate_environment() -> Dict[str, Any]:
            ...
            try:
                ...
                if venv_exists:
                    try:
                        result = await asyncio.create_subprocess_exec(
                            "uv", "run", "python", "-c", "import ruff, pyright, pytest",
                            stdout=asyncio.subprocess.PIPE,
                            stderr=asyncio.subprocess.PIPE
                        )
                        await result.communicate()
                ...
                self.logger.info(f"Environment validation complete: {'ready' if status['environment_ready'] else 'not ready'}")
        ...
        for line in lines:
            if ":" in line and ".py:" in line:
                ...
                parsed["issues"].append({
                    "type": "lint",
                    "line": line,
                    "severity": "error" if "E" in line else "warning"
                })
