...
@cli.command()
@click.option("--force", is_flag=True, help="Force full rebuild even if up-to-date")
def reconcile(force: bool):
    ...
    # Quick check (unless --force)
    if not force and _is_jsonl_fresh(extractions_dir, jsonl_path):
        logger.info("JSONL is up-to-date, skipping reconciliation")
        return

    # Full rebuild
    start_time = time.time()
    stats = _rebuild_jsonl(extractions_dir, jsonl_path)
    elapsed = time.time() - start_time

    # Report results
    logger.info(f"Reconciliation complete: {stats['processed']} files processed in {elapsed:.1f} seconds")
    if stats["errors"]:
        logger.warning(f"Warnings: {stats['errors']} files skipped due to errors")


def _is_jsonl_fresh(extractions_dir: Path, jsonl_path: Path) -> bool:
    """Check if JSONL is reasonably up-to-date.

    Args:
        extractions_dir: Directory containing individual extraction files
        jsonl_path: Path to the JSONL file

    Returns:
        True if JSONL appears fresh, False if rebuild needed
    """
    if not jsonl_path.exists():
        return False

    # Count files in extractions directory
    extraction_files = list(extractions_dir.glob("*.json"))
    file_count = len(extraction_files)

    # Count lines in JSONL (each line is one record)
    try:
    ...
    if file_count != jsonl_count:
        logger.info(f"Count mismatch: {file_count} extraction files vs {jsonl_count} JSONL records")
        return False

    # Check if JSONL was modified recently (< 5 minutes)
    jsonl_mtime = jsonl_path.stat().st_mtime
    current_time = time.time()
    age_minutes = (current_time - jsonl_mtime) / 60

    if age_minutes < 5:
        return True

    # JSONL is older than 5 minutes, might be stale
    logger.info(f"JSONL is {age_minutes:.1f} minutes old, will reconcile")
    return False


def _rebuild_jsonl(extractions_dir: Path, jsonl_path: Path):
    """Rebuild JSONL from individual extraction files.

    Args:
        extractions_dir: Directory containing individual extraction files
        jsonl_path: Path to write the JSONL file

    Returns:
        Stats dict with 'processed' and 'errors' counts
    """
    stats = {"processed": 0, "errors": 0}

    # Ensure extraction directory exists
    if not extractions_dir.exists():
        logger.info("No extractions directory found, nothing to reconcile")
        return stats

    # Get all extraction files
    extraction_files = sorted(extractions_dir.glob("*.json"))
    if not extraction_files:
        logger.info("No extraction files found, nothing to reconcile")
        return stats

    logger.info(f"Reconciling {len(extraction_files)} files...")

    # Backup existing JSONL if it exists
    if jsonl_path.exists():
        backup_path = jsonl_path.with_suffix(".jsonl.bak")
        jsonl_path.rename(backup_path)
        logger.info(f"Backed up existing JSONL to {backup_path}")
    ...
    with open(jsonl_path, "w", encoding="utf-8") as out_file:
        for extraction_file in extraction_files:
            try:
                # Read individual extraction file
                with open(extraction_file, encoding="utf-8") as f:
                    individual_data = json.load(f)

                # Transform to JSONL format (matching ArticleProcessor logic)
                jsonl_data = {
                    "source_id": individual_data.get("article_id"),
                    "extracted_at": individual_data.get("extracted_at", datetime.now().isoformat()),
                    "success": True,
                }

                # Flatten the nested data structure from 'data' field
                if "data" in individual_data:
                    jsonl_data.update(individual_data["data"])
                else:
                    # Fallback if structure is different
                    logger.warning(f"Unexpected format in {extraction_file.name}, using as-is")
                    jsonl_data.update(individual_data)

                # Write to JSONL
                out_file.write(json.dumps(jsonl_data, ensure_ascii=False) + "\n")
                stats["processed"] += 1

            except Exception as e:
                logger.warning(f"Failed to process {extraction_file.name}: {e}")
                stats["errors"] += 1
                continue
