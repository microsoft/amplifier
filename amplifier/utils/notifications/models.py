...
class NotificationRequest(BaseModel):
    ...
    subtitle: str | None = Field(
        default=None, description="Optional subtitle (e.g., project name)"
    )
...
class NotificationResponse(BaseModel):
    ...
    fallback_used: bool = Field(
        default=False, description="Whether console fallback was used"
    )
    error: str | None = Field(default=None, description="Error message if failed")
    debug_log: str | None = Field(
        default=None, description="Debug information if debug mode enabled"
    )
...
class ClaudeCodeHookInput(BaseModel):
    ...
    hook_event_name: str | None = Field(
        default=None, description="Name of the hook event"
    )
